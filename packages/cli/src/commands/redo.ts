...
/**
 * `diagram redo` plus the auto-serve hook (spec §9.1).
 *
 * redo IS a document-changing write, and it is one the user is
 ...
 * written for, an redo that restores them is the same case running backwards,
 * so it is treated identically. The content gate in autoServe keeps the
 * degenerate direction quiet: an redo that lands on the empty document opens
 * no window, because there is nothing on it to look at.
 */
export async function runRedoServing(
  opts: ContextOptions & ServeControl = {},
): Promise<CommandOutput> {
  const ctx = createContext({ ...(opts.dir !== undefined ? { dir: opts.dir } : {}) });
  const out = runRedo({ ...(opts.dir !== undefined ? { dir: opts.dir } : {}) });
